...
class globalGet
{
	...
	private function repetiteurInfos()
	{
		...
		$this->buffer .= '<name>' . $this->prepareText($row['forename'] . ' ' . $row['name']) . '</name>';
		$this->buffer .= '<placeComment><![CDATA[' . $this->prepareText($row['placecomment'] ?? '') . ']]></placeComment>'; // commentaire de lieu
