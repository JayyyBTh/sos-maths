...
function registerUptMain_displayForm($rep)
{
	...
	foreach($texts as $key => $value)
		$tpl->assign_var($value, htmlspecialchars($rep->data[ $value ], ENT_QUOTES | ENT_HTML5, 'UTF-8'));
...
function termMain_displayForm($rep)
{
	...
	foreach($globalVariables['fee'] as $text => $data) 
		$tpl->assign_block_vars('feeOpt', array('value' => $data[0] . '_' . $data[1], 'checked' => (in_array($data[0] . '_' . $data[1], $rep->data['fee']) ? 'checked="checked"' : ''), 'caption' => htmlspecialchars($text, ENT_QUOTES | ENT_HTML5, 'UTF-8')));
