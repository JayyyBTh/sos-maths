...
function writeDisplayForm($stu)
{
	...
	$tpl->assign_var('EMAIL', htmlspecialchars($stu->data['emailText'], ENT_QUOTES | ENT_HTML5, 'UTF-8'));
	$tpl->assign_var('ID', $stu->id);
	$tpl->assign_var('NAME', $stu->data['forename'] . ' ' . $stu->data['name']);
	
	$texts = array('emailContact', 'telContact');
	foreach($texts as $key => $value)
		$tpl->assign_var($value, htmlspecialchars($stu->data[ $value ], ENT_QUOTES | ENT_HTML5, 'UTF-8'));
	
	$a = rand(1, 9);
	$b = rand(1, 9);
	$_SESSION['math_captcha_answer'] = $a + $b;
	$tpl->assign_var('CAPTCHA_QUESTION', "$a + $b");
...
function mainMain()
{
	...
	foreach($ret as $row)
	{
		...
		$tpl->assign_block_vars('row', array('NAME' => $row['forename'] . ' ' . $row['name'],
			'ID' => $row['id'],
			'FEES' => $fees,
