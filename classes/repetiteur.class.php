...
class repetiteur
{
	...
	*/
	public function verifyData()
	{
		...
		if (!$q->execute(array($this->data['name'], $this->data['forename'])))
		{
			$this->error = 'GET_DATA';
			return false;
		}
		...
		if ($this->do == 'add')
		{
			if (!isset($_POST['captcha_code']) || !isset($_SESSION['math_captcha_answer'])
				|| (int)$_POST['captcha_code'] !== $_SESSION['math_captcha_answer'])
			{
	...
	public function addDB()
	{
		...
		foreach($texts as $value)
		{
			...
			$values_data[ $value ] = $this->data[ $value ];
		}
	...
	public function uptDB()
	{
		...
		foreach($texts as $key => $value)
		{
			...
			$q_params[ $value ] = $this->data[ $value  ];
		}
