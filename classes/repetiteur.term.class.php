	...
	*/
	public function setDefaultValues()
	{
		...
		if ($this->do == 'upt')
		{
			...
			$this->data['placeComment'] = count($ret) ? $ret[0][0] : '';
	...
	*/
	...
	public function doDB()
	{
		...
		if ($this->do == 'add')
		{
			...
			if (!$q->execute(array('userId' => $this->userId, 'annee' => $this->term[0], 'term' => $this->term[1], 'fee' => $this->data['feeIndex'], 'feeMin' => $this->data['feeMin'], 'feeMax' => $this->data['feeMax'], 'available' => ($this->data['available'] ? '1' : '0'), 'commentaire' =>  $this->data['comment'])))
			{
				$this->error = 'GET_DATA';
				return false;
			}
		...
		else if ($this->do == 'upt')
		{
			...
			if (!$q->execute(array($this->data['feeMin'], $this->data['feeMax'], ($this->data['available'] ? '1' : '0'), $this->data['comment'], $this->id)))
			{
				$pdo->rollBack();
				$this->error = 'GET_DATA';
				return false;
		...
		if (!empty($this->data['placeComment']))
		{
			...
			if (!$q->execute(array($this->id, $this->data['placeComment'])))
			{
				$pdo->rollBack();
				$this->error = 'GET_DATA';
				return false;
